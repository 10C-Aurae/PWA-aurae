...
import {
  Plus, Search, SlidersHorizontal, X, CalendarX, Ticket,
  MapPin, Users, FlaskConical, ChefHat, Handshake, Palette, Gamepad2, Leaf,
} from 'lucide-react'
  ...
  const filtered = useMemo(() => {
    ...
    return todos.filter((ev) => {
      ...
      if (timeTab === 'proximos') {
        ...
        if (end ? now > end : now > start) return false
      ...
      if (debouncedQ) {
        ...
        const inLoc  = (typeof ev.ubicacion === 'string' ? ev.ubicacion : ev.ubicacion?.nombre ?? '')
                         .toLowerCase().includes(debouncedQ)
  ...
  return (
    <div className="page">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        {/* ── Header: greeting + create CTA ── */}
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="page-title">Hola, {user?.nombre?.split(' ')[0] ?? 'viajero'}</h1>
            <p className="page-subtitle">Descubre eventos o crea el tuyo</p>
          </div>
          <button
            ...
            <div className="flex-1 min-w-0">
              ...
              {arquetipo && (
                <p className="text-[11px] text-aura-muted truncate">
                  {(() => {
                    const Icon = ARCHETYPE_ICONS[arquetipo.iconName]
                    return Icon ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
            ...
            <p className="font-bold text-aura-ink">
              {debouncedQ || catFiltro
                ...
                : timeTab === 'proximos' ? 'No hay eventos próximos'
                : timeTab === 'pasados'  ? 'No hay eventos pasados'
                : 'Sin eventos por ahora'}
            </p>
            <p className="mt-1 text-sm text-aura-muted max-w-xs">
              {debouncedQ || catFiltro
                ? 'Prueba con otros términos o limpia los filtros'
                : 'Crea el primero y comparte la experiencia'}
            </p>
            {(debouncedQ || catFiltro) ? (
              <button
                onClick={() => { setCatFiltro(null); setQuery('') }}
